...
$("save-settings").onclick=async()=>{try{let settings={};["stock_target_days","vendor_lead_time_days"].forEach(n=>settings[n]=num($(n).value));await api("/api/settings",settings);status("settings-status","Settings saved.","success");await load()}catch(error){status("settings-status",error.message,"error")}};
