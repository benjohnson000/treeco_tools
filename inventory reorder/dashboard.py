...
<header class="brand-header"><img class="brand-logo" src="/assets/treeco-logo.png" alt="Treeco"><div><p class="eyebrow">Inventory management</p><h1>Spruce Reorder Tool</h1><div class="hint">Import reports, review branch-level recommendations, then export a consolidated order.</div></div></header><style>.settings{grid-template-columns:repeat(4,minmax(160px,1fr)) auto}</style>
<section class="panel"><details><summary><b>Reorder settings</b></summary><div class="grid settings" style="margin-top:12px"><label>Stock target (days)<input id="stock_target_days" type="number" min="0" step="1"></label><label>Vendor lead time (days)<input id="vendor_lead_time_days" type="number" min="0" step="1"></label><label>Sales location remapping<select id="location_remap_enabled"><option value="false">Disabled</option><option value="true">Enabled</option></select></label><button id="save-settings">Save settings</button></div><div id="remap-settings" class="grid settings hidden" style="margin-top:12px"><label>Report sales from locations<select id="location_remap_sources" multiple></select></label><label>As sales for location<select id="location_remap_target"></select></label></div><div id="remap-help" class="hint hidden" style="margin-top:8px">Sales from the selected locations are added to the target location. The selected source locations will not appear as order locations.</div><div id="settings-status" class="status"></div></details></section>
...
$("save-settings").onclick=async()=>{try{let settings={};["stock_target_days","vendor_lead_time_days"].forEach(n=>settings[n]=num($(n).value));settings.location_remap_enabled=$("location_remap_enabled").value==="true";settings.location_remap_sources=[...$("location_remap_sources").selectedOptions].map(option=>option.value);settings.location_remap_target=$("location_remap_target").value;await api("/api/settings",settings);status("settings-status","Settings saved.","success");await load()}catch(error){status("settings-status",error.message,"error")}};
...
load().then(()=>api("/api/state").then(data=>{configuredBranches=data.configured_branches;loadRemapSettings(data.settings)}).catch(()=>{}));
</script></body></html>"""
