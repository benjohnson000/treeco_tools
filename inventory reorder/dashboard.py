...
function updateRemapVisibility(){let show=$("location_remap_enabled").value==="true";$("remap-settings").classList.toggle("hidden",!show);$("remap-help").classList.toggle("hidden",!show)}
function loadRemapSettings(settings){let entries=Object.entries(configuredBranches),sources=new Set((settings.location_remap_sources||[]).map(String));$("location_remap_enabled").value=String(Boolean(settings.location_remap_enabled));$("location_remap_sources").innerHTML=entries.map(x=>"<option value='"+esc(x[0])+"' "+(sources.has(String(x[0]))?"selected":"")+">"+esc(x[0])+" - "+esc(x[1])+"</option>").join("");$("location_remap_target").innerHTML=entries.map(x=>"<option value='"+esc(x[0])+"' "+(String(x[0])===String(settings.location_remap_target)?"selected":"")+">"+esc(x[0])+" - "+esc(x[1])+"</option>").join("");updateRemapVisibility()}
async function load(){try{let data=await api("/api/state");rows=data.rows;branches=data.branches;configuredBranches=data.configured_branches;vendors=data.vendors;["stock_target_days","vendor_lead_time_days"].forEach(n=>$(n).value=data.settings[n]);$("branches").innerHTML=Object.entries(branches).map(x=>"<option value='"+esc(x[0])+"' selected>"+esc(x[0])+" — "+esc(x[1])+"</option>").join("");$("import-branch").innerHTML=Object.entries(configuredBranches).map(x=>"<option value='"+esc(x[0])+"'>"+esc(x[0])+" — "+esc(x[1])+"</option>").join("");let used=[...new Set(rows.map(r=>String(r.vendor||"")))].sort((a,b)=>String(vendors[a]||a).localeCompare(String(vendors[b]||b)));$("vendors").innerHTML=used.map(code=>"<option value='"+esc(code)+"' selected>"+esc(code?code+" — ":"")+" "+esc(vendors[code]||(code?"Unknown vendor":"No vendor assigned"))+"</option>").join("");$("load-error").textContent="";render()}catch(error){$("load-error").textContent=error.message}}
...
$("download").onclick=async()=>{let items=chosen();if(!items.length)return;try{let response=await fetch("/api/export",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({items:items,branch_ids:branchIds()})});if(!response.ok){let data=await response.json();throw Error(data.error||"Export failed")}let contents=await response.text();if(hasDesktopApi()){let saved=await window.pywebview.api.save_order_csv(contents);status("load-error",saved?"Order saved to: "+saved:"Save cancelled.","success")}else{let link=document.createElement("a");link.href=URL.createObjectURL(new Blob([contents],{type:"text/csv;charset=utf-8"}));link.download="purchase_order_draft.csv";link.click();URL.revokeObjectURL(link.href)}}catch(error){$("load-error").textContent=error.message}};
function populateVendorFilter(){let codes=[...new Set(["",...Object.keys(vendors),...rows.map(row=>String(row.vendor||""))])].sort((left,right)=>!left?-1:!right?1:left.localeCompare(right,undefined,{numeric:true}));$("vendors").innerHTML=codes.map(code=>"<option value='"+esc(code)+"' selected>"+esc(code?code+" - ":"")+esc(vendors[code]||(code?"Unknown vendor":"Unassigned / no vendor mapping"))+"</option>").join("");render()}
load().then(()=>api("/api/state").then(data=>{configuredBranches=data.configured_branches;vendors=data.vendors;loadRemapSettings(data.settings);populateVendorFilter()}).catch(()=>{}));
</script></body></html>"""
