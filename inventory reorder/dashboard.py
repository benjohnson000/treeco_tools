...
function populateVendorFilter(){let codes=[...new Set(["",...Object.keys(vendors),...rows.map(row=>String(row.vendor||""))])].sort((left,right)=>String(vendors[left]||left||"Unassigned").localeCompare(String(vendors[right]||right||"Unassigned")));$("vendors").innerHTML=codes.map(code=>"<option value='"+esc(code)+"' selected>"+esc(code?code+" - ":"")+esc(vendors[code]||(code?"Unknown vendor":"Unassigned / no vendor mapping"))+"</option>").join("");render()}
