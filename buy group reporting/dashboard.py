...
document.getElementById('upload-buy-groups').onclick=()=>upload('buy-groups-file','/api/buy-groups','Account Number vs Buy Group');document.getElementById('upload-sales').onclick=()=>upload('sales-file','/api/sales','sales data');document.getElementById('apply-filter').onclick=loadReport;descriptionFilter.addEventListener('input',loadReport);hideZeroUnitPrice.addEventListener('change',loadReport);loadFullButton.onclick=()=>{showFullTable=true;renderTable()};document.getElementById('download-report').onclick=()=>{try{if(!currentReport.rows.length)throw Error('There are no displayed rows to download.');const quote=value=>'"'+String(value??'').replace(/"/g,'""')+'"';const itemIndex=currentReport.headers.indexOf('Item');const excelSku=value=>'="'+String(value??'').replace(/"/g,'""')+'"';const exportRows=currentReport.rows.map(row=>row.map((value,index)=>index===itemIndex?excelSku(value):value));const csv=[currentReport.headers,...exportRows].map(row=>row.map(quote).join(',')).join('\r\n');const link=document.createElement('a');link.href=URL.createObjectURL(new Blob([csv],{type:'text/csv;charset=utf-8'}));link.download='buy_group_report.csv';link.click();URL.revokeObjectURL(link.href)}catch(error){setStatus(error.message,'error')}};loadReport();
</script></body></html>"""
