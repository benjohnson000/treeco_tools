...
<script>
const status=document.getElementById('status'),groups=document.getElementById('buy-groups'),locations=document.getElementById('locations'),descriptionFilter=document.getElementById('description-filter'),hideZeroUnitPrice=document.getElementById('hide-zero-unit-price'),report=document.getElementById('report'),head=document.getElementById('report-head'),tableStatus=document.getElementById('table-status'),loadFullButton=document.getElementById('load-full-table'),loadedSalesFile=document.getElementById('loaded-sales-file'),columnSettings=document.getElementById('column-settings');let currentReport={headers:[],rows:[],sales_file:null},showFullTable=false,columnPreferences=[];
    ...
    def do_GET(self):
        ...
        if request.path == "/":
            ...
            try:
                ...
                self.json(200, report(query.get("group", []), query.get("location", []), query.get("description", [""])[0], query.get("exclude_zero_unit_price", ["0"])[0] == "1"))
...
def location_code(value):
    """Use the first four-digit location where a mapping lists multiple locations."""
    value = clean(value)
    return value[:4] if len(value) >= 4 and value[:4].isdigit() else value


def report(selected_groups, selected_locations, description_filter="", exclude_zero_unit_price=False):
    mapping = buy_group_mapping()
    groups = sorted({group for group, _location in mapping.values()} | {"Unassigned"})
    locations = sorted({location_code(location) for _group, location in mapping.values() if location_code(location)})
    if not SALES_DATABASE.exists(): return {"groups": groups, "locations": locations, "headers": REPORT_HEADERS, "rows": [], "sales_file": None}
    with sqlite3.connect(SALES_DATABASE) as database:
        database.row_factory = sqlite3.Row
        columns = {row[1] for row in database.execute("PRAGMA table_info(sales)")}
        if "raw_data" not in columns: return {"groups": groups, "locations": locations, "headers": REPORT_HEADERS, "rows": [], "sales_file": None}
    ...
    for row in rows:
        ...
        if selected_locations and location_code(location) not in selected_locations: continue
    ...
    return {"groups": groups, "locations": locations, "headers": REPORT_HEADERS, "rows": result, "sales_file": import_row["source_file"] if import_row else None}
