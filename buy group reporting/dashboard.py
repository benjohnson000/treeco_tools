...
<script>
const status=document.getElementById('status'),groups=document.getElementById('buy-groups'),descriptionFilter=document.getElementById('description-filter'),hideZeroUnitPrice=document.getElementById('hide-zero-unit-price'),report=document.getElementById('report'),head=document.getElementById('report-head'),tableStatus=document.getElementById('table-status'),loadFullButton=document.getElementById('load-full-table'),loadedSalesFile=document.getElementById('loaded-sales-file'),columnSettings=document.getElementById('column-settings');let currentReport={headers:[],rows:[],sales_file:null},showFullTable=false,columnPreferences=[];
const esc=v=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
function setStatus(message,kind=''){status.textContent=message;status.className='status '+kind}
function loadColumnPreferences(){try{return JSON.parse(localStorage.getItem('treeco-buy-group-columns'))||[]}catch{return[]}}
function saveColumnPreferences(){localStorage.setItem('treeco-buy-group-columns',JSON.stringify(columnPreferences))}
function configureColumns(){const saved=loadColumnPreferences(),savedByName=new Map(saved.map(column=>[column.name,column]));columnPreferences=[...saved.filter(column=>currentReport.headers.includes(column.name)),...currentReport.headers.filter(name=>!savedByName.has(name)).map(name=>({name,visible:true}))];renderColumnSettings()}
...
function renderColumnSettings(){columnSettings.innerHTML=columnPreferences.map((column,index)=>`<div class="actions" style="margin:8px 0"><label style="min-width:210px"><input class="check" type="checkbox" data-column="${esc(column.name)}" ${column.visible?'checked':''}> ${esc(column.name)}</label><button class="secondary" data-move="up" data-index="${index}" ${index===0?'disabled':''}>Up</button><button class="secondary" data-move="down" data-index="${index}" ${index===columnPreferences.length-1?'disabled':''}>Down</button></div>`).join('');columnSettings.querySelectorAll('input[data-column]').forEach(input=>input.onchange=()=>{const column=columnPreferences.find(column=>column.name===input.dataset.column);if(!input.checked&&visibleColumns().length===1){input.checked=true;setStatus('At least one column must remain visible.','error');return}column.visible=input.checked;saveColumnPreferences();renderTable()});columnSettings.querySelectorAll('button[data-move]').forEach(button=>button.onclick=()=>{const index=Number(button.dataset.index),next=index+(button.dataset.move==='up'?-1:1);[columnPreferences[index],columnPreferences[next]]=[columnPreferences[next],columnPreferences[index]];saveColumnPreferences();renderColumnSettings();renderTable()})}
    ...
    def do_GET(self):
        ...
        if request.path == "/":
            ...
            try:
                ...
                self.json(200, report(query.get("group", []), query.get("description", [""])[0], query.get("exclude_zero_unit_price", ["0"])[0] == "1"))
...
def report(selected_groups, description_filter="", exclude_zero_unit_price=False):
    ...
    with sqlite3.connect(SALES_DATABASE) as database:
        ...
        if "raw_data" not in columns: return {"groups": groups, "headers": REPORT_HEADERS, "rows": [], "sales_file": None}
