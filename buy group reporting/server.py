"""Railway web server for the Buy Group Reporting tool."""
...
class ProtectedHandler(Handler):
    ...
    def require_auth(self):
        ...
        self.send_header("WWW-Authenticate", 'Basic realm="Treeco Buy Group Reporting"')
...
def main():
    if os.environ.get("RAILWAY_ENVIRONMENT") and not os.environ.get("APP_PASSWORD"):
        raise RuntimeError("Set APP_PASSWORD before deploying this internal reporting tool.")
    port = int(os.environ.get("PORT", "8080"))
    print(f"Buy Group Reporting listening on port {port}")
